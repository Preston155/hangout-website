...
const TYPE_STYLE = {
  slash: "border-sky-500/30 bg-sky-500/10 text-sky-200",
  prefix: "border-violet-500/30 bg-violet-500/10 text-violet-200",
  auto: "border-emerald-500/30 bg-emerald-500/10 text-emerald-200",
} as const;
...
export function DirectoryCommandCard({ command, index, accent }: { command: BotCommand; index: number; accent: string }) {
  ...
  return (
    <motion.article
      ...
      transition={{ delay: Math.min(index * 0.018, 0.24), duration: 0.28 }}
      whileHover={{ y: -4 }}
      className="group relative overflow-hidden rounded-[26px] border border-white/[.08] bg-white/[.035] p-4 shadow-[0_24px_70px_-42px_rgba(0,0,0,.9)] transition hover:border-white/[.14] hover:bg-white/[.055] sm:p-5"
      style={{ boxShadow: `0 24px 70px -48px ${accent}` }}
    >
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/25 to-transparent" />
      <div className="absolute -right-16 -top-16 h-36 w-36 rounded-full opacity-0 blur-3xl transition duration-500 group-hover:opacity-35" style={{ background: accent }} />

      <div className="relative flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl border border-white/10 bg-black/20" style={{ color: accent }}><Terminal size={15} /></span>
      ...
      <div className="relative mt-4 flex flex-wrap gap-2">
        <span className={`rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-wide ${TYPE_STYLE[command.type]}`}>{TYPE_LABELS[command.type]}</span>
        ...
        <span className="rounded-full border border-white/[.07] bg-black/20 px-2.5 py-1 text-[10px] font-bold text-zinc-500">{command.category}</span>
      </div>

      <div className="relative mt-4 rounded-2xl border border-white/[.07] bg-black/20 p-3">
        <div className="text-[10px] font-black uppercase tracking-[.16em] text-zinc-600">Usage</div>
