...
function printSaleReceipt(sale: TireSale, onBlocked: () => void) {
  ...
  const workName = isTire ? "Tire Purchase" : workTypeLabel(sale.serviceType);
  ...
  const itemDetails = isTire
    ? `<div class="item"><div><div class="item-name">${printEscape(sale.size)}</div><div class="item-detail">${printEscape(tirePackageLabel(sale.packageType))} · ${sale.quantity} tire${sale.quantity === 1 ? "" : "s"}</div></div><div class="item-price">${printEscape(money(sale.total))}</div></div>`
    : `<div class="item"><div><div class="item-name">${printEscape(workTypeLabel(sale.serviceType))}</div><div class="item-detail">Quantity ${sale.quantity}</div></div><div class="item-price">${printEscape(money(sale.total))}</div></div>`;
  receiptWindow.document.write(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Receipt ${printEscape(receiptNumber)}</title><style>
    @page{size:80mm auto;margin:5mm}*{box-sizing:border-box}body{margin:0;background:#eef1ef;color:#161916;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif;font-size:12px}.receipt{width:min(360px,100%);margin:20px auto;background:#fff;border:1px solid #dfe4e1;border-radius:16px;overflow:hidden;box-shadow:0 16px 45px rgba(20,30,24,.13)}.accent{height:6px;background:#22c98b}.content{padding:24px}.brand-row{display:flex;align-items:center;gap:9px}.logo{display:grid;width:29px;height:29px;place-items:center;border-radius:8px;background:#171a18;color:#55e2ae;font-weight:850}.brand{font-size:13px;font-weight:850;letter-spacing:.1em}.receipt-title{margin:22px 0 0;font-size:24px;letter-spacing:-.035em}.sub{margin-top:4px;color:#737d77;font-size:10px}.meta{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:20px;padding:14px;border-radius:11px;background:#f4f7f5}.label{font-size:8px;font-weight:800;text-transform:uppercase;letter-spacing:.11em;color:#86908a}.meta-value{margin-top:3px;font-size:10px;font-weight:700}.section-label{margin:22px 0 8px;font-size:8px;font-weight:800;text-transform:uppercase;letter-spacing:.12em;color:#86908a}.item{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;padding:14px 0;border-top:1px solid #e3e7e5;border-bottom:1px solid #e3e7e5}.item-name{font-size:15px;font-weight:800}.item-detail{margin-top:4px;color:#707973;font-size:10px}.item-price{font-size:16px;font-weight:850;white-space:nowrap}.detail-list{display:grid;gap:9px;margin-top:14px}.detail{display:flex;justify-content:space-between;gap:18px;color:#68716c}.detail b{color:#202521;text-align:right}.total{display:flex;align-items:flex-end;justify-content:space-between;margin-top:20px;padding:16px;border-radius:11px;background:#171a18;color:#fff}.total-label{font-size:9px;font-weight:750;text-transform:uppercase;letter-spacing:.1em;color:#aeb7b2}.total-value{font-size:25px;font-weight:850;color:#55e2ae;letter-spacing:-.03em}.thanks{margin-top:22px;text-align:center}.thanks b{display:block;font-size:12px}.thanks span{display:block;margin-top:4px;color:#8a938e;font-size:9px}.actions{display:flex;gap:8px;width:min(360px,calc(100% - 24px));margin:0 auto 20px}.actions button{flex:1;border:0;border-radius:10px;padding:12px;font:inherit;font-weight:750}.print{background:#171a18;color:#fff}.close{border:1px solid #d8ddda!important;background:#fff;color:#343b37}@media(max-width:420px){.receipt{width:100%;margin:0;border:0;border-radius:0;box-shadow:none}.actions{position:sticky;bottom:0;width:100%;margin:0;padding:12px;background:rgba(238,241,239,.95);backdrop-filter:blur(10px)}}@media print{body{background:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact}.receipt{width:100%;margin:0;border:0;border-radius:0;box-shadow:none}.content{padding:0}.actions{display:none}}
  </style></head><body><main class="receipt"><div class="accent"></div><div class="content"><div class="brand-row"><div class="logo">P</div><div class="brand">PRESTONHQ</div></div><h1 class="receipt-title">Sales Receipt</h1><div class="sub">Receipt #${printEscape(receiptNumber)}</div><section class="meta"><div><div class="label">Date</div><div class="meta-value">${printEscape(soldDate)}</div></div><div><div class="label">Time</div><div class="meta-value">${printEscape(soldTime)} ET</div></div></section><div class="section-label">${printEscape(workName)}</div>${itemDetails}<section class="detail-list"><div class="detail"><span>Payment</span><b>${printEscape(sale.paymentMethod)}</b></div><div class="detail"><span>Customer</span><b>${printEscape(sale.customer || "Walk-in")}</b></div>${sale.notes ? `<div class="detail"><span>Notes</span><b>${printEscape(sale.notes)}</b></div>` : ""}</section><div class="total"><div class="total-label">Total charged</div><div class="total-value">${printEscape(money(sale.total))}</div></div><div class="thanks"><b>Thank you</b><span>Receipt generated by PrestonHQ</span></div></div></main><div class="actions"><button class="close" onclick="window.close()">Close</button><button class="print" onclick="window.print()">Print Receipt</button></div><script>window.addEventListener('load',()=>setTimeout(()=>window.print(),180));<\/script></body></html>`);
