...
function TireSalesPage({ showToast, setPage }: { showToast: (m: string) => void; setPage: (p: Page) => void }) {
  ...
  const recordSale = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const next = await api<TireShopData>(path, { method: editingSaleId ? "PATCH" : "POST", body: JSON.stringify({ ...form, quantity: Number(form.quantity), totalPrice: Number(form.unitPrice), soldAt }) });
      setData(next);
