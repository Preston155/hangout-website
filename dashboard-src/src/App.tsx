...
function TireSalesPage({ showToast, setPage }: { showToast: (m: string) => void; setPage: (p: Page) => void }) {
  const [data, setData] = useState<TireShopData | null>(null);
  const [form, setForm] = useState({ inventoryId: "", quantity: "1", unitPrice: "", soldDate: easternDateKey(new Date()), soldTime: easternTimeValue(), customer: "", paymentMethod: "Cash", notes: "", adjustInventory: true });
  ...
  const editSale = (sale: TireSale) => {
    ...
    setForm({ inventoryId: sale.inventoryId, quantity: String(sale.quantity), unitPrice: String(sale.unitPrice), soldDate, soldTime: easternTimeValue(sale.soldAt), customer: sale.customer, paymentMethod: ["Cash", "Cashapp", "Chime"].includes(sale.paymentMethod) ? sale.paymentMethod : "Cash", notes: sale.notes, adjustInventory: soldDate === easternDateKey(new Date()) });
  ...
  const removeSale = async (sale: TireSale) => {
    if (!window.confirm(`Remove this ${sale.size} sale?${sale.adjustInventory === false ? "" : " The sold quantity will be returned to inventory."}`)) return;
    setBusy(true);
    try {
