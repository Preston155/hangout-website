...
function TireSalesReportPage({ showToast, setPage }: { showToast: (m: string) => void; setPage: (p: Page) => void }) {
  ...
  const printReport = (reportSales: TireSale[], reportTitle: string, includeDetails: boolean) => {
    ...
    const rows = reportSales.map((sale) => {
      ...
      return `<tr><td>${printEscape(new Date(sale.soldAt).toLocaleString("en-US", { timeZone: "America/New_York", month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" }))}</td><td>${printEscape(work)}</td><td>${printEscape(quantityLabel)}</td><td>${printEscape(sale.paymentMethod)}</td><td>${printEscape(sale.customer || "Walk-in")}</td><td class="money">${printEscape(money(sale.total))}</td></tr>`;
    }).join("");
    printWindow.document.write(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${printEscape(reportTitle)}</title><style>@page{size:auto;margin:.55in}*{box-sizing:border-box}body{margin:0;color:#171717;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif;font-size:12px}header{display:flex;justify-content:space-between;gap:24px;align-items:flex-end;border-bottom:2px solid #171717;padding-bottom:14px}h1{margin:0;font-size:24px}h2{margin:24px 0 10px;font-size:15px}.muted{color:#666;margin-top:5px}.brand{font-size:13px;font-weight:800;letter-spacing:.08em}.summary{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:18px}.box{border:1px solid #d4d4d4;border-radius:8px;padding:12px}.label{font-size:9px;text-transform:uppercase;letter-spacing:.08em;color:#666}.value{font-size:20px;font-weight:750;margin-top:5px}.payments{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.payment{display:flex;justify-content:space-between;border:1px solid #ddd;border-radius:7px;padding:10px;font-weight:650}table{width:100%;border-collapse:collapse;margin-top:8px}th{text-align:left;font-size:9px;text-transform:uppercase;letter-spacing:.07em;color:#666;border-bottom:1px solid #999;padding:8px 6px}td{padding:9px 6px;border-bottom:1px solid #e5e5e5;vertical-align:top}.money{text-align:right;font-weight:700;white-space:nowrap}footer{margin-top:24px;padding-top:10px;border-top:1px solid #ddd;color:#777;font-size:9px}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body><header><div><div class="brand">PRESTONHQ</div><h1>${printEscape(reportTitle)}</h1><div class="muted">Sales & services report</div></div><div class="muted">Printed ${printEscape(new Date().toLocaleString("en-US"))}</div></header><section class="summary"><div class="box"><div class="label">Total revenue</div><div class="value">${printEscape(money(reportRevenue))}</div></div><div class="box"><div class="label">Tires sold</div><div class="value">${reportTires}</div></div><div class="box"><div class="label">Transactions</div><div class="value">${reportSales.length}</div></div></section><h2>Payment breakdown</h2><section class="payments">${["Cash", "Cashapp", "Chime"].map((method) => `<div class="payment"><span>${method}</span><span>${printEscape(money(paymentMethodTotal(reportSales, method)))}</span></div>`).join("")}</section>${includeDetails ? `<h2>Transaction details</h2><table><thead><tr><th>Date & time</th><th>Work</th><th>Quantity</th><th>Payment</th><th>Customer</th><th class="money">Total</th></tr></thead><tbody>${rows}</tbody></table>` : ""}<footer>Generated from PrestonHQ • ${printEscape(reportTitle)}</footer><script>window.addEventListener('load',()=>setTimeout(()=>window.print(),150));<\/script></body></html>`);
