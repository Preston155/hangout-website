...
function TireSalesReportPage({ showToast, setPage }: { showToast: (m: string) => void; setPage: (p: Page) => void }) {
  ...
  const printReport = (reportSales: TireSale[], reportTitle: string, includeDetails: boolean) => {
    const printWindow = window.open("", "_blank", "width=900,height=720");
    if (!printWindow) {
      showToast("Allow pop-ups for PrestonHQ to print reports.");
      return;
    }
    const reportRevenue = reportSales.reduce((sum, sale) => sum + Number(sale.total || 0), 0);
    const reportTires = physicalTireCount(reportSales);
    const rows = reportSales.map((sale) => {
      const isTire = (sale.serviceType || "tire") === "tire";
      const work = isTire ? `${sale.size} — ${tirePackageLabel(sale.packageType)}` : workTypeLabel(sale.serviceType);
      const quantityLabel = isTire ? `${sale.quantity} tire${sale.quantity === 1 ? "" : "s"}` : `${sale.quantity} ${workTypeLabel(sale.serviceType)}`;
      const date = new Date(sale.soldAt).toLocaleDateString("en-US", { timeZone: "America/New_York", month: "short", day: "numeric", year: "numeric" });
      const time = new Date(sale.soldAt).toLocaleTimeString("en-US", { timeZone: "America/New_York", hour: "numeric", minute: "2-digit" });
      return `<article class="transaction"><div class="transaction-top"><div class="work-block"><span class="type-badge ${isTire ? "tire" : "service"}">${isTire ? "Tire sale" : "Service"}</span><div class="work-name">${printEscape(work)}</div><div class="work-quantity">${printEscape(quantityLabel)}</div></div><div class="transaction-total">${printEscape(money(sale.total))}</div></div><div class="transaction-meta"><span><b>${printEscape(date)}</b> · ${printEscape(time)}</span><span>${printEscape(sale.paymentMethod)}</span><span>${printEscape(sale.customer || "Walk-in")}</span></div></article>`;
    }).join("");
    const printedAt = new Date().toLocaleString("en-US", { month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" });
    printWindow.document.write(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${printEscape(reportTitle)}</title><style>
      @page{size:letter;margin:.45in}*{box-sizing:border-box}body{margin:0;background:#eef1ef;color:#171a18;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif;font-size:12px;line-height:1.4}.page{width:min(820px,calc(100% - 24px));margin:22px auto;background:#fff;border:1px solid #dde2df;border-radius:18px;overflow:hidden;box-shadow:0 18px 50px rgba(20,30,24,.12)}.accent{height:7px;background:#22c98b}.content{padding:28px 30px 24px}.report-header{display:flex;align-items:flex-start;justify-content:space-between;gap:28px;padding-bottom:22px;border-bottom:1px solid #dde2df}.identity{display:flex;align-items:center;gap:10px;margin-bottom:18px}.logo{display:grid;place-items:center;width:30px;height:30px;border-radius:8px;background:#171a18;color:#55e2ae;font-size:14px;font-weight:850}.brand{font-size:12px;font-weight:850;letter-spacing:.12em}.eyebrow,.label{font-size:9px;font-weight:750;text-transform:uppercase;letter-spacing:.12em;color:#758079}.report-header h1{max-width:510px;margin:5px 0 0;font-size:27px;line-height:1.08;letter-spacing:-.035em}.report-kind{margin-top:7px;color:#657069}.print-meta{min-width:145px;padding:11px 13px;border-radius:10px;background:#f4f7f5;color:#657069;font-size:10px}.print-meta b{display:block;margin-bottom:3px;color:#29302c;font-size:10px}.summary{display:grid;grid-template-columns:1.35fr 1fr 1fr;gap:10px;margin-top:18px}.box{min-height:84px;padding:14px;border:1px solid #dde2df;border-radius:12px;background:#fbfcfb}.box.primary{border-color:#bcebd8;background:#edfbf5}.value{margin-top:6px;font-size:22px;font-weight:800;letter-spacing:-.025em}.primary .value{color:#087a52}.section-heading{display:flex;align-items:center;justify-content:space-between;margin:24px 0 10px}.section-heading h2{margin:0;font-size:14px;letter-spacing:-.01em}.section-heading span{font-size:9px;text-transform:uppercase;letter-spacing:.1em;color:#8a938e}.payments{display:grid;grid-template-columns:repeat(3,1fr);gap:9px}.payment{display:flex;justify-content:space-between;gap:10px;padding:11px 12px;border:1px solid #dde2df;border-radius:10px;background:#fbfcfb}.payment span:first-child{color:#606a64}.payment span:last-child{font-weight:750}.transactions{display:grid;gap:8px}.transaction{padding:13px 14px;border:1px solid #dde2df;border-radius:11px;break-inside:avoid;background:#fff}.transaction-top{display:flex;align-items:flex-start;justify-content:space-between;gap:18px}.work-block{min-width:0}.type-badge{display:inline-block;margin-bottom:6px;padding:3px 7px;border-radius:999px;font-size:8px;font-weight:800;text-transform:uppercase;letter-spacing:.09em}.type-badge.tire{background:#edf1ef;color:#535d57}.type-badge.service{background:#e8faf3;color:#087a52}.work-name{font-size:13px;font-weight:750}.work-quantity{margin-top:2px;color:#6f7873;font-size:10px}.transaction-total{font-size:16px;font-weight:800;white-space:nowrap}.transaction-meta{display:flex;flex-wrap:wrap;gap:5px 14px;margin-top:10px;padding-top:9px;border-top:1px solid #edf0ee;color:#707974;font-size:9px}.transaction-meta span:not(:last-child):after{content:"";display:inline-block;width:3px;height:3px;margin-left:14px;border-radius:50%;background:#9ea6a1;vertical-align:middle}.report-footer{display:flex;justify-content:space-between;gap:16px;margin-top:24px;padding-top:12px;border-top:1px solid #dde2df;color:#8a938e;font-size:9px}.screen-actions{display:flex;justify-content:flex-end;gap:8px;margin:0 auto 22px;width:min(820px,calc(100% - 24px))}.screen-actions button{border:0;border-radius:10px;padding:11px 16px;font:inherit;font-weight:750;cursor:pointer}.screen-actions .print{background:#171a18;color:#fff}.screen-actions .close{background:#fff;color:#39413d;border:1px solid #d8ddda}
      @media(max-width:600px){.page{width:100%;margin:0;border:0;border-radius:0;box-shadow:none}.content{padding:22px 18px}.report-header{display:block}.print-meta{margin-top:15px;min-width:0}.report-header h1{font-size:24px}.summary{grid-template-columns:1.25fr 1fr 1fr}.box{min-height:76px;padding:11px}.value{font-size:18px}.payments{grid-template-columns:1fr}.payment{padding:10px 12px}.transaction{padding:12px}.transaction-meta{gap:5px 9px}.transaction-meta span:not(:last-child):after{margin-left:9px}.screen-actions{position:sticky;bottom:0;width:100%;margin:0;padding:12px;background:rgba(238,241,239,.94);backdrop-filter:blur(10px)}}
      @media print{body{background:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact}.page{width:100%;margin:0;border:0;border-radius:0;box-shadow:none}.content{padding:0}.screen-actions{display:none}.transaction{break-inside:avoid}.report-footer{position:relative}}
    </style></head><body><main class="page"><div class="accent"></div><div class="content"><header class="report-header"><div><div class="identity"><div class="logo">P</div><div class="brand">PRESTONHQ</div></div><div class="eyebrow">Sales report</div><h1>${printEscape(reportTitle)}</h1><div class="report-kind">Tire sales, shop services, and payment activity</div></div><div class="print-meta"><b>Report generated</b>${printEscape(printedAt)}<br>Eastern Time</div></header><section class="summary"><div class="box primary"><div class="label">Total revenue</div><div class="value">${printEscape(money(reportRevenue))}</div></div><div class="box"><div class="label">Tires sold</div><div class="value">${reportTires}</div></div><div class="box"><div class="label">Transactions</div><div class="value">${reportSales.length}</div></div></section><div class="section-heading"><h2>Payment breakdown</h2><span>Collected revenue</span></div><section class="payments">${["Cash", "Cashapp", "Chime"].map((method) => `<div class="payment"><span>${method}</span><span>${printEscape(money(paymentMethodTotal(reportSales, method)))}</span></div>`).join("")}</section>${includeDetails ? `<div class="section-heading"><h2>Transaction details</h2><span>${reportSales.length} entries</span></div><section class="transactions">${rows}</section>` : ""}<footer class="report-footer"><span>PrestonHQ sales records</span><span>${printEscape(reportTitle)}</span></footer></div></main><div class="screen-actions"><button class="close" onclick="window.close()">Close</button><button class="print" onclick="window.print()">Print or Save PDF</button></div><script>window.addEventListener('load',()=>setTimeout(()=>window.print(),180));<\/script></body></html>`);
    printWindow.document.close();
  };

  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-2xl font-semibold tracking-tight text-white">Monthly Sales Report</h1><p className="mt-1 text-xs text-zinc-400">All tire sales and services. Updates automatically every 10 seconds.</p>{lastUpdated && <p className="mt-1 text-[10px] text-zinc-600">Last updated {lastUpdated.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", second: "2-digit" })}</p>}</div><div className="flex flex-wrap gap-2"><Button variant="ghost" disabled={refreshing} onClick={() => void load()}><RefreshCw className={`h-3.5 w-3.5 ${refreshing ? "animate-spin" : ""}`} /> Refresh</Button><Button variant="secondary" disabled={!sales.length} onClick={() => printReport(sales, `${monthKeyLabel(monthKey)} Summary`, false)}><Printer className="h-3.5 w-3.5" /> Print Summary</Button><Button variant="secondary" disabled={!sales.length} onClick={() => printReport(sales, `${monthKeyLabel(monthKey)} Full Report`, true)}><FileText className="h-3.5 w-3.5" /> Print Full</Button><Button variant="secondary" onClick={() => setPage("tire-sales")}><ShoppingCart className="h-3.5 w-3.5" /> Record Work</Button></div></div>
      ...
      <Card>
        ...
        {!data ? <EmptyState title="Loading sales and services" text="Reading saved monthly records..." /> : days.length === 0 ? <EmptyState title="No work recorded this month" text="Use the arrows to check another month." /> : <div className="mt-4 space-y-4">{days.map(([date, daySales]) => { const dayTotal = daySales.reduce((sum, sale) => sum + sale.total, 0); const dayTires = physicalTireCount(daySales); const dayLabel = new Date(`${date}T12:00:00`).toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" }); return <section key={date} className="overflow-hidden rounded-xl border border-zinc-800"><div className="flex items-center justify-between gap-3 border-b border-zinc-800 bg-zinc-950/70 px-4 py-3"><div><div className="text-sm font-semibold text-white">{new Date(`${date}T12:00:00`).toLocaleDateString("en-US", { weekday: "long", month: "short", day: "numeric" })}</div><div className="mt-0.5 text-[10px] text-zinc-500">{dayTires} tire{dayTires === 1 ? "" : "s"} sold • {daySales.length} transaction{daySales.length === 1 ? "" : "s"}</div></div><div className="flex items-center gap-2"><button onClick={() => printReport(daySales, dayLabel, true)} className="grid h-9 w-9 place-items-center rounded-lg border border-zinc-700 text-zinc-400 transition hover:bg-zinc-800 hover:text-white" aria-label={`Print ${dayLabel}`}><Printer className="h-3.5 w-3.5" /></button><div className="text-base font-semibold text-emerald-300">{money(dayTotal)}</div></div></div><div className="divide-y divide-zinc-800/70">{daySales.map((sale) => { const isTire = (sale.serviceType || "tire") === "tire"; return <div key={sale.id} className="grid gap-2 px-4 py-3 text-xs sm:grid-cols-[1fr_auto_auto] sm:items-center"><div><div className="flex flex-wrap items-center gap-2"><span className={`font-semibold text-zinc-200 ${isTire ? "font-mono" : ""}`}>{isTire ? sale.size : workTypeLabel(sale.serviceType)}</span><span className={`rounded border px-1.5 py-0.5 text-[9px] font-semibold ${isTire ? tirePackageClass(sale.packageType) : "border-cyan-500/25 bg-cyan-500/10 text-cyan-300"}`}>{isTire ? tirePackageLabel(sale.packageType) : "Service"}</span></div><div className="mt-1 text-[10px] text-zinc-500">{new Date(sale.soldAt).toLocaleTimeString("en-US", { timeZone: "America/New_York", hour: "numeric", minute: "2-digit" })} • {sale.customer || "Walk-in"} • {sale.paymentMethod}</div></div><div className="text-zinc-400">{isTire ? `${sale.quantity} tire${sale.quantity === 1 ? "" : "s"}` : `${sale.quantity} ${workTypeLabel(sale.serviceType)}`}</div><div className="font-semibold text-white sm:text-right">{money(sale.total)}</div></div>; })}</div></section>; })}</div>}
      </Card>
    </div>
  );
}
