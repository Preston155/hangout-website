...
function printSaleReceipt(sale: TireSale, onBlocked: () => void) {
  const receiptWindow = window.open("", "_blank", "width=460,height=780");
  if (!receiptWindow) {
    onBlocked();
    return;
  }
  ...
  const itemDetails = isTire
    ? `<div class="item"><div class="item-copy"><div class="item-name">${printEscape(sale.size)}</div><div class="item-detail">${printEscape(tirePackageLabel(sale.packageType))}</div><div class="quantity">Qty: ${sale.quantity} tire${sale.quantity === 1 ? "" : "s"}</div></div><div class="item-price">${printEscape(money(sale.total))}</div></div>`
    : `<div class="item"><div class="item-copy"><div class="item-name">${printEscape(workTypeLabel(sale.serviceType))}</div><div class="item-detail">Automotive service</div><div class="quantity">Qty: ${sale.quantity}</div></div><div class="item-price">${printEscape(money(sale.total))}</div></div>`;
  receiptWindow.document.write(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Akron Tire Shop Receipt ${printEscape(receiptNumber)}</title><style>
    @page{size:80mm auto;margin:4mm}*{box-sizing:border-box}body{margin:0;background:#e9ecea;color:#141714;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif;font-size:12px}.receipt{width:min(382px,100%);margin:22px auto;background:#fff;border:1px solid #d9dedb;border-radius:14px;overflow:hidden;box-shadow:0 18px 50px rgba(15,24,18,.14)}.header{position:relative;padding:24px 24px 22px;background:#141714;color:#fff}.header:after{content:"";position:absolute;right:0;bottom:0;left:0;height:4px;background:#20c985}.brand-row{display:flex;align-items:center;gap:12px}.mark{display:grid;width:42px;height:42px;place-items:center;border:1px solid #3b4640;border-radius:50%;color:#35dc98;font-size:11px;font-weight:900;letter-spacing:-.04em}.brand{font-size:17px;font-weight:850;letter-spacing:-.025em}.tagline{margin-top:2px;color:#aeb7b2;font-size:9px;font-weight:650;letter-spacing:.08em;text-transform:uppercase}.receipt-heading{display:flex;align-items:flex-end;justify-content:space-between;gap:12px;margin-top:24px}.receipt-title{margin:0;font-size:23px;letter-spacing:-.035em}.paid{border:1px solid #35dc98;border-radius:999px;padding:5px 9px;color:#55e4a8;font-size:8px;font-weight:850;letter-spacing:.12em}.content{padding:22px 24px 24px}.receipt-number{padding-bottom:16px;border-bottom:1px dashed #cfd5d1;color:#77807b;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:9px;letter-spacing:.06em}.meta{display:grid;grid-template-columns:1.4fr 1fr;gap:16px;padding:17px 0;border-bottom:1px dashed #cfd5d1}.label{font-size:8px;font-weight:850;text-transform:uppercase;letter-spacing:.13em;color:#8a938e}.meta-value{margin-top:4px;font-size:11px;font-weight:750}.section-label{margin:20px 0 8px;font-size:8px;font-weight:850;text-transform:uppercase;letter-spacing:.13em;color:#8a938e}.item{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;padding:15px;border:1px solid #e0e4e2;border-radius:10px;background:#fafbfa}.item-copy{min-width:0}.item-name{font-size:17px;font-weight:850;letter-spacing:-.02em}.item-detail{margin-top:4px;color:#5f6963;font-size:10px;font-weight:650}.quantity{margin-top:8px;color:#7c8580;font-size:9px}.item-price{font-size:17px;font-weight:850;white-space:nowrap}.detail-list{display:grid;gap:11px;margin-top:18px;padding:0 2px 18px;border-bottom:1px dashed #cfd5d1}.detail{display:flex;justify-content:space-between;gap:18px;color:#747d78;font-size:10px}.detail b{max-width:62%;color:#202521;text-align:right}.total{display:flex;align-items:center;justify-content:space-between;margin-top:18px;padding:18px;border-radius:10px;background:#effaf5;border:1px solid #ccebdc}.total-label{font-size:9px;font-weight:850;text-transform:uppercase;letter-spacing:.11em;color:#47705e}.total-value{font-size:27px;font-weight:900;color:#10764c;letter-spacing:-.04em}.thanks{margin-top:22px;text-align:center}.thanks b{display:block;font-size:13px}.thanks span{display:block;margin-top:5px;color:#858e89;font-size:9px}.footer-rule{width:34px;height:3px;margin:16px auto 0;border-radius:99px;background:#20c985}.actions{display:flex;gap:9px;width:min(382px,calc(100% - 24px));margin:0 auto 22px}.actions button{flex:1;border:0;border-radius:10px;padding:13px;font:inherit;font-weight:800;cursor:pointer}.print{background:#141714;color:#fff}.close{border:1px solid #ccd2ce!important;background:#fff;color:#343b37}@media(max-width:440px){body{background:#fff}.receipt{width:100%;margin:0;border:0;border-radius:0;box-shadow:none}.actions{position:sticky;bottom:0;width:100%;margin:0;padding:12px;background:rgba(255,255,255,.94);backdrop-filter:blur(10px)}}@media print{body{background:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact}.receipt{width:100%;margin:0;border:0;border-radius:0;box-shadow:none}.header{padding:18px 16px}.content{padding:18px 16px 0}.actions{display:none}}
  </style></head><body><main class="receipt"><header class="header"><div class="brand-row"><div class="mark">ATS</div><div><div class="brand">Akron Tire Shop</div><div class="tagline">Tires &amp; Automotive Service</div></div></div><div class="receipt-heading"><h1 class="receipt-title">Customer Receipt</h1><span class="paid">PAID</span></div></header><div class="content"><div class="receipt-number">RECEIPT #${printEscape(receiptNumber)}</div><section class="meta"><div><div class="label">Date</div><div class="meta-value">${printEscape(soldDate)}</div></div><div><div class="label">Time</div><div class="meta-value">${printEscape(soldTime)}</div></div></section><div class="section-label">${printEscape(workName)}</div>${itemDetails}<section class="detail-list"><div class="detail"><span>Payment method</span><b>${printEscape(sale.paymentMethod)}</b></div><div class="detail"><span>Customer</span><b>${printEscape(sale.customer || "Walk-in customer")}</b></div>${sale.notes ? `<div class="detail"><span>Details</span><b>${printEscape(sale.notes)}</b></div>` : ""}</section><div class="total"><div class="total-label">Total Paid</div><div class="total-value">${printEscape(money(sale.total))}</div></div><div class="thanks"><b>Thank you for your business.</b><span>Keep this receipt for your records.</span><div class="footer-rule"></div></div></div></main><div class="actions"><button class="close" onclick="window.close()">Close</button><button class="print" onclick="window.print()">Print Receipt</button></div><script>window.addEventListener('load',()=>setTimeout(()=>window.print(),180));<\/script></body></html>`);
...
function TireSalesPage({ showToast, setPage }: { showToast: (m: string) => void; setPage: (p: Page) => void }) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><h1 className="text-2xl font-semibold tracking-tight text-white">Sales & Services</h1><p className="mt-1 text-xs text-zinc-400">Record tire sales, mount and balance work, plugs, rotations, and brakes.</p></div><Button variant="secondary" onClick={() => setPage("tire-inventory")}><Package className="h-3.5 w-3.5" /> Open Inventory</Button></div>
