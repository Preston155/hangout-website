...
export function App() {
  ...
  return (
    ...
    <div className="relative min-h-screen overflow-x-hidden bg-[#080908] font-sans text-zinc-100 antialiased selection:bg-orange-500/30 selection:text-white">
      <div className="pointer-events-none fixed inset-0 opacity-40" style={{ backgroundImage: "linear-gradient(rgba(255,255,255,.025) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.025) 1px,transparent 1px)", backgroundSize: "42px 42px", maskImage: "linear-gradient(to bottom,black,transparent 75%)" }} />
      <div className="pointer-events-none fixed -right-56 -top-56 h-[520px] w-[520px] rounded-full bg-orange-500/[.07] blur-3xl" />
      <div className="flex min-h-screen">
        <Sidebar page={page} setPage={setPage} />
        <main className="relative flex min-w-0 flex-1 flex-col lg:pl-64">
          <Topbar page={page} />
          <div className="mx-auto w-full max-w-[1280px] flex-1 p-4 pb-28 sm:p-6 sm:pb-28 lg:p-8 lg:pb-8">
...
function LoadingScreen() {
  return (
    <div className="grid min-h-screen place-items-center bg-[#080908] text-zinc-400">
      <div className="flex flex-col items-center gap-3">
        <div className="h-7 w-7 animate-spin rounded-full border-2 border-zinc-800 border-t-orange-500" />
        <span className="text-[10px] font-bold uppercase tracking-[.18em]">Opening shop floor...</span>
      </div>
    </div>
  );
}
...
function LoginScreen({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    <div className="relative grid min-h-screen place-items-center overflow-hidden bg-[#080908] p-4 text-zinc-100">
      ...
      <div className="pointer-events-none absolute -left-40 top-1/3 h-96 w-96 rounded-full bg-orange-500/10 blur-3xl" />
      <div className="relative grid w-full max-w-5xl overflow-hidden rounded-2xl border border-white/[.08] bg-[#0d0e0d]/95 shadow-[0_32px_100px_rgba(0,0,0,.65)] lg:grid-cols-[1.1fr_.9fr]">
        <section className="relative hidden min-h-[580px] flex-col justify-between overflow-hidden border-r border-white/[.07] p-10 lg:flex">
          <div className="absolute -bottom-28 -right-28 h-80 w-80 rounded-full border-[56px] border-white/[.025]" />
          <div><div className="h-1 w-14 bg-orange-500" /><div className="mt-6 text-[11px] font-black uppercase tracking-[.28em] text-orange-500">Akron Tire Shop</div></div>
          <div><div className="text-6xl font-black uppercase leading-[.88] tracking-[-.07em] text-white">Built for<br /><span className="text-zinc-600">the shop</span><br />floor.</div><p className="mt-7 max-w-sm text-sm leading-relaxed text-zinc-500">Inventory, services, sales, and daily numbers—organized in one hard-working system.</p></div>
          <div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-[.16em] text-zinc-600"><span className="h-2 w-2 rounded-full bg-orange-500" /> Private management system</div>
        </section>
        <section className="p-5 sm:p-8 lg:flex lg:flex-col lg:justify-center lg:p-10">
          <div className="mb-7 lg:hidden"><div className="h-1 w-10 bg-orange-500" /><div className="mt-4 text-[10px] font-black uppercase tracking-[.22em] text-orange-500">Akron Tire Shop</div><h1 className="mt-2 text-3xl font-black uppercase tracking-[-.045em] text-white">Shop Access</h1></div>
          <div className="mb-7 hidden lg:block"><div className="text-[10px] font-black uppercase tracking-[.2em] text-orange-500">Authorized staff only</div><h1 className="mt-2 text-3xl font-black tracking-[-.04em] text-white">Welcome back.</h1><p className="mt-2 text-xs text-zinc-500">Sign in to open the shop dashboard.</p></div>
          <form onSubmit={login} className="rounded-xl border border-white/[.07] bg-white/[.025] p-5">
          <div className="space-y-4">
            {passkeyAvailable && <><button type="button" disabled={busy} onClick={() => void loginWithPasskey()} className="flex w-full items-center justify-center gap-2 rounded-lg bg-orange-500 py-3 text-sm font-black text-black transition hover:bg-orange-400 active:scale-[.99] disabled:opacity-40"><KeyRound className="h-4 w-4" />{busy ? "Waiting for passkey..." : "Unlock with a passkey"}</button><p className="-mt-1 text-center text-[10px] leading-relaxed text-zinc-600">Windows Hello, security key, or phone passkey</p><div className="flex items-center gap-3"><span className="h-px flex-1 bg-white/[.07]" /><span className="text-[9px] font-bold uppercase tracking-[.15em] text-zinc-700">or use shop passcode</span><span className="h-px flex-1 bg-white/[.07]" /></div></>}
            <div>
              <label className="mb-1.5 block text-[10px] font-bold uppercase tracking-[.13em] text-zinc-500">Shop Passcode</label>
              <input autoFocus={!passkeyAvailable} type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••••••" className="w-full rounded-lg border border-white/[.09] bg-[#080908] px-3.5 py-3 text-sm text-zinc-100 placeholder-zinc-700 outline-none transition focus:border-orange-500/60 focus:ring-2 focus:ring-orange-500/10" />
            </div>
            {error && <div className="rounded-lg border border-red-500/20 bg-red-500/10 p-2.5 text-xs text-red-400">{error}</div>}
            <button disabled={busy || !password} className="w-full rounded-lg border border-white/[.1] bg-white py-3 text-xs font-black uppercase tracking-[.08em] text-black transition hover:bg-zinc-200 active:scale-[.99] disabled:opacity-40">
              {busy ? "Opening..." : "Enter Shop"}
            </button>
          </div>
          </form>
        </section>
...
function Sidebar({ page, setPage }: { page: Page; setPage: (p: Page) => void }) {
  return (
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col border-r border-white/[.07] bg-[#0b0c0b]/95 px-4 py-5 backdrop-blur-xl lg:flex">
      <div className="px-2 py-2">
        <div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center bg-orange-500 font-black text-black">AT</div><div><div className="text-sm font-black uppercase tracking-[-.02em] text-white">Akron Tire</div><div className="text-[9px] font-bold uppercase tracking-[.2em] text-orange-500">Shop system</div></div></div>
      </div>
      <div className="my-5 h-px bg-white/[.07]" />
      <div className="mb-2 px-3 text-[8px] font-black uppercase tracking-[.2em] text-zinc-700">Shop navigation</div>
      <nav className="space-y-1">
        {navItems.map((item) => {
          const Icon = item.icon;
          const active = item.id === page;
          return (
            <button key={item.id} onClick={() => setPage(item.id)} className={`group relative flex w-full items-center gap-3 overflow-hidden rounded-lg px-3 py-3 text-left text-xs font-bold transition ${active ? "bg-white/[.07] text-white" : "text-zinc-600 hover:bg-white/[.035] hover:text-zinc-300"}`}>
              {active && <span className="absolute inset-y-2 left-0 w-0.5 bg-orange-500" />}
              <Icon className={`h-4 w-4 ${active ? "text-orange-500" : "text-zinc-700 group-hover:text-zinc-500"}`} />
              <span>{item.label}</span>
              <span className={`ml-auto text-[8px] font-black ${active ? "text-orange-500" : "text-zinc-800"}`}>0{navItems.indexOf(item) + 1}</span>
            </button>
          );
        })}
      </nav>
      <div className="mt-auto border-t border-white/[.07] px-2 pt-5">
        <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.08em] text-zinc-400"><span className="relative flex h-2 w-2"><span className="absolute h-full w-full animate-ping rounded-full bg-green-500 opacity-50" /><span className="relative h-2 w-2 rounded-full bg-green-500" /></span> Shop data live</div>
        <p className="mt-2 text-[9px] leading-relaxed text-zinc-700">Every inventory and sales change saves automatically.</p>
      </div>
    </aside>
  );
}
...
function Topbar({ page }: { page: Page }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center border-b border-white/[.07] bg-[#080908]/85 px-4 backdrop-blur-xl sm:px-6 lg:px-8">
      <div className="lg:hidden"><div className="text-[8px] font-black uppercase tracking-[.2em] text-orange-500">Akron Tire Shop</div><div className="mt-0.5 text-sm font-black uppercase tracking-tight text-white">{currentNav?.label}</div></div>
      <div className="hidden lg:flex lg:items-center lg:gap-4"><div className="h-8 w-1 bg-orange-500" /><div><div className="text-sm font-black uppercase tracking-tight text-white">{currentNav?.label}</div><div className="mt-0.5 text-[9px] font-bold uppercase tracking-[.14em] text-zinc-700">Shop management console</div></div></div>
      <div className="ml-auto flex items-center gap-2 rounded-full border border-white/[.07] bg-white/[.025] px-3 py-1.5 text-[9px] font-bold uppercase tracking-[.08em] text-zinc-500"><span className="h-1.5 w-1.5 rounded-full bg-green-500" /><span>Auto-saved</span></div>
    </header>
  );
}
...
function MobileShopNav({ page, setPage }: { page: Page; setPage: (p: Page) => void }) {
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-white/[.08] bg-[#0a0b0a]/95 px-1 pb-[max(8px,env(safe-area-inset-bottom))] pt-2 shadow-[0_-16px_40px_rgba(0,0,0,.35)] backdrop-blur-xl lg:hidden">
      <div className="mx-auto grid max-w-lg grid-cols-5">
        {navItems.map((item) => { const Icon = item.icon; const active = item.id === page; return <button key={item.id} onClick={() => setPage(item.id)} className={`flex min-w-0 flex-col items-center gap-1 rounded-lg px-1 py-1.5 text-[9px] font-bold transition ${active ? "bg-orange-500/10 text-orange-400" : "text-zinc-700"}`}><Icon className={`h-[18px] w-[18px] ${active ? "text-orange-500" : "text-zinc-700"}`} /><span className="truncate">{item.shortLabel}</span></button>; })}
      </div>
    </nav>
  );
}
            ...
            {data.giveaways.length === 0 ? <EmptyState title="No active giveaways" text="New Veltrix giveaways will appear here automatically." /> : data.giveaways.map((giveaway) => (
              <div key={giveaway.id} className="rounded-lg border border-zinc-800 bg-zinc-950/50 p-3.5">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="rounded-full border border-orange-400/20 bg-orange-400/10 px-2 py-0.5 text-[9px] font-semibold uppercase text-orange-300">{giveaway.status}</span>
                </div>
...
function SettingsPage({ showToast }: { showToast: (m: string) => void }) {
  ...
  return (
    <div className="space-y-6">
      <div><div className="mb-1 text-[9px] font-black uppercase tracking-[.2em] text-orange-500">Access control</div><h1 className="text-2xl font-black uppercase tracking-[-.045em] text-white sm:text-3xl">Account</h1><p className="mt-1 text-xs text-zinc-600">Manage secure access to Akron Tire Shop.</p></div>
      <section className="overflow-hidden rounded-xl border border-white/[.08] bg-white/[.025]">
        <div className="border-b border-white/[.07] px-5 py-4"><div className="text-sm font-black uppercase text-white">Sign-in security</div><div className="mt-1 text-[11px] text-zinc-500">Use Windows Hello, Face ID, a passkey, or your shop passcode.</div></div>
        <div className="p-5">
          <div className="flex flex-col gap-4 rounded-lg border border-orange-500/20 bg-orange-500/[.06] p-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-3"><div className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-orange-500/10"><KeyRound className="h-4 w-4 text-orange-300" /></div><div><div className="text-xs font-medium text-white">Device sign-in</div><div className="mt-1 max-w-xl text-[11px] leading-relaxed text-zinc-500">On Windows, this creates a Windows Hello passkey unlocked with your PC PIN. iPhone passkeys use Face ID.</div><div className="mt-1.5 text-[10px] font-medium text-orange-400">{passkeyCount} passkey{passkeyCount === 1 ? "" : "s"} registered</div></div></div>
            <button disabled={passkeyBusy} onClick={() => void addPasskey()} className="shrink-0 rounded-lg bg-orange-400 px-4 py-2.5 text-xs font-semibold text-zinc-950 transition hover:bg-orange-300 disabled:opacity-40">{passkeyBusy ? "Waiting for Windows..." : isWindows ? "Set Up Windows PIN" : passkeyCount > 0 ? "Add Another Passkey" : "Add a Passkey"}</button>
          </div>
        </div>
      </section>
...
const tireFieldClass = "w-full rounded-lg border border-white/[.09] bg-[#090a09] px-3 py-2.5 text-xs text-zinc-100 placeholder-zinc-700 outline-none transition focus:border-orange-500/60 focus:ring-2 focus:ring-orange-500/10";
...
function tirePackageClass(value?: string) {
  if (value === "set4") return "border-orange-500/25 bg-orange-500/10 text-orange-300";
  if (value === "pair") return "border-sky-500/25 bg-sky-500/10 text-sky-300";
...
function TireStat({ label, value, detail }: { label: string; value: string | number; detail: string }) {
  return (
    <div className="relative min-w-0 overflow-hidden rounded-xl border border-white/[.07] bg-[#101110]/90 p-3 shadow-[0_12px_32px_rgba(0,0,0,.18)] sm:p-4">
      <span className="absolute left-0 top-4 h-8 w-0.5 bg-orange-500" />
      <div className="truncate text-[9px] font-black uppercase tracking-[.12em] text-zinc-600 sm:text-[10px]">{label}</div>
      <div className="mt-1 truncate text-xl font-black tracking-[-.04em] text-white sm:mt-1.5 sm:text-2xl">{value}</div>
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div><div className="mb-1 text-[9px] font-black uppercase tracking-[.2em] text-orange-500">Stock control</div><h1 className="text-2xl font-black uppercase tracking-[-.045em] text-white sm:text-3xl">Manage Inventory</h1><p className="mt-1 text-xs text-zinc-600">Add tires, update stock, and keep pricing organized.</p></div>
        <Button className="w-full justify-center sm:w-auto" variant="secondary" onClick={() => setPage("tire-sales")}><ShoppingCart className="h-3.5 w-3.5" /> Open Tire Sales</Button>
      </div>
      ...
      <Card>
        <CardHeader title={editingId ? "Edit Inventory Item" : "Add Inventory Item"} icon={<Package className="h-4 w-4 text-orange-400" />} />
        <form onSubmit={saveItem} className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="flex gap-2 sm:col-span-2 lg:col-span-4">
            <button disabled={busy} className="w-full rounded-lg bg-orange-400 px-4 py-3 text-xs font-semibold text-zinc-950 transition hover:bg-orange-300 disabled:opacity-50 sm:w-auto sm:py-2.5">{busy ? "Saving..." : editingId ? "Save Changes" : "Add to Inventory"}</button>
      ...
      <Card>
        <CardHeader title="All Inventory" icon={<Package className="h-4 w-4 text-zinc-400" />} action={<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search inventory..." className="w-full rounded-lg border border-zinc-800 bg-zinc-950 px-3 py-2.5 text-xs text-zinc-200 outline-none focus:border-orange-500/50 sm:w-64 sm:py-2" />} />
        {!data ? <EmptyState title="Loading inventory" text="Reading the tire shop database..." /> : inventory.length === 0 ? <EmptyState title="No tires found" text={search ? "Try another search." : "Use the form above to enter your first inventory item."} /> : (<>
          <div className="mt-4 space-y-3 sm:hidden">
            {inventory.map((item) => <article key={item.id} className="rounded-xl border border-white/[.07] bg-[#0d100e] p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="truncate font-mono text-lg font-semibold text-white">{item.size}</div><span className={`mt-2 inline-flex rounded-md border px-2 py-1 text-[10px] font-semibold ${tirePackageClass(item.packageType)}`}>{tirePackageLabel(item.packageType)}</span></div><div className="shrink-0 text-right"><div className={`text-2xl font-semibold ${item.quantity <= 5 ? "text-amber-300" : "text-orange-300"}`}>{item.quantity}</div><div className="text-[9px] uppercase tracking-wider text-zinc-600">in stock</div></div></div><div className="mt-4 grid grid-cols-[1fr_44px_44px] items-end gap-2 border-t border-white/[.07] pt-3"><div><div className="text-[9px] font-semibold uppercase tracking-wider text-zinc-600">Selling price</div><div className="mt-1 text-lg font-semibold text-zinc-100">{money(item.price)}</div></div><button disabled={adjustingId !== null || item.quantity === 0} onClick={() => void adjustItem(item, -1)} className="grid h-11 place-items-center rounded-lg border border-zinc-700 bg-zinc-900 text-sm font-semibold text-zinc-200 disabled:cursor-not-allowed disabled:opacity-35" aria-label={`Remove one ${item.size} tire`}>−1</button><button disabled={adjustingId !== null} onClick={() => void adjustItem(item, 1)} className="grid h-11 place-items-center rounded-lg border border-orange-500/25 bg-orange-500/10 text-sm font-semibold text-orange-300 disabled:cursor-not-allowed disabled:opacity-35" aria-label={`Add one ${item.size} tire`}>+1</button></div><button disabled={busy} onClick={() => void removeItem(item)} className="mt-2.5 w-full rounded-lg border border-red-500/15 py-2 text-[10px] font-medium text-red-400/80">Remove from inventory</button></article>)}
          </div>
...
function TireInventoryViewPage({ showToast }: { showToast: (m: string) => void }) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div className="mb-1 text-[9px] font-black uppercase tracking-[.2em] text-orange-500">Live showroom</div><h1 className="text-2xl font-black uppercase tracking-[-.045em] text-white sm:text-3xl">Inventory</h1><p className="mt-1 text-xs text-zinc-600">Available tires and current selling prices.</p></div><button onClick={() => void load()} className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-white/[.09] bg-white/[.035] px-3 py-2.5 text-xs font-bold text-zinc-300 hover:bg-white/[.07] sm:w-auto sm:py-2"><RefreshCw className="h-3.5 w-3.5 text-orange-500" /> Refresh Stock</button></div>
      ...
      <Card>
        <CardHeader title="Available Inventory" icon={<Package className="h-4 w-4 text-zinc-400" />} action={<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search tire size..." className="w-full rounded-lg border border-zinc-800 bg-zinc-950 px-3 py-2.5 text-xs text-zinc-200 outline-none focus:border-orange-500/50 sm:w-64 sm:py-2" />} />
        {!data ? <EmptyState title="Loading inventory" text="Reading current stock..." /> : inventory.length === 0 ? <EmptyState title="No tires found" text={search ? "Try another tire size." : "There is no inventory to display yet."} /> : <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{inventory.map((item) => <article key={item.id} className="rounded-xl border border-zinc-800 bg-zinc-950/50 p-4"><div className="flex items-start justify-between gap-3"><div><div className="font-mono text-lg font-semibold text-white">{item.size}</div><span className={`mt-2 inline-flex rounded-md border px-2 py-1 text-[10px] font-semibold ${tirePackageClass(item.packageType)}`}>{tirePackageLabel(item.packageType)}</span></div><div className="text-right"><div className={`text-2xl font-semibold ${item.quantity === 0 ? "text-red-400" : item.quantity <= 5 ? "text-amber-300" : "text-orange-300"}`}>{item.quantity}</div><div className="text-[10px] text-zinc-500">available</div></div></div><div className="mt-4 flex items-end justify-between border-t border-zinc-800 pt-3"><div><div className="text-[9px] font-semibold uppercase tracking-wider text-zinc-600">Selling price</div><div className="mt-1 text-lg font-semibold text-zinc-100">{money(item.price)}</div></div><div className={`rounded-md px-2 py-1 text-[9px] font-semibold uppercase tracking-wider ${item.quantity === 0 ? "bg-red-500/10 text-red-400" : item.quantity <= 5 ? "bg-amber-500/10 text-amber-300" : "bg-orange-500/10 text-orange-300"}`}>{item.quantity === 0 ? "Out of stock" : item.quantity <= 5 ? "Low stock" : "In stock"}</div></div></article>)}</div>}
      </Card>
    </div>
  );
}
...
function TireSalesPage({ showToast, setPage }: { showToast: (m: string) => void; setPage: (p: Page) => void }) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div className="mb-1 text-[9px] font-black uppercase tracking-[.2em] text-orange-500">Front counter</div><h1 className="text-2xl font-black uppercase tracking-[-.045em] text-white sm:text-3xl">Sales & Services</h1><p className="mt-1 text-xs text-zinc-600">Record tire sales, mount and balance work, plugs, rotations, and brakes.</p></div><Button className="w-full justify-center sm:w-auto" variant="secondary" onClick={() => setPage("tire-inventory")}><Package className="h-3.5 w-3.5 text-orange-500" /> Open Inventory</Button></div>
      ...
      <section className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-4 sm:p-5">
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">{(["Cash", "Cashapp", "Chime"] as const).map((method) => <button key={method} onClick={() => setSelectedPayment(method)} className={`min-w-0 rounded-xl border p-2.5 text-left sm:p-3 transition ${selectedPayment === method ? "border-orange-500/35 bg-orange-500/10" : "border-zinc-800 bg-zinc-950/40 hover:border-zinc-700"}`}><div className={`text-[10px] font-semibold uppercase tracking-wider ${selectedPayment === method ? "text-orange-400" : "text-zinc-500"}`}>{method}</div><div className="mt-1 truncate text-base font-semibold text-white sm:text-xl">{money(paymentMethodTotal(paymentSales, method))}</div></button>)}</div>
        <div className="mt-3 flex items-center justify-between rounded-lg bg-zinc-950/60 px-3 py-2.5 text-xs"><span className="text-zinc-500">Selected: {selectedPayment}</span><span className="font-semibold text-orange-300">{money(selectedPaymentTotal)}</span></div>
      </section>
      <Card>
        <CardHeader title={editingSaleId ? "Edit Work Entry" : "Record Work"} icon={<ShoppingCart className="h-4 w-4 text-orange-400" />} action={<div className="text-right"><div className="text-[9px] font-semibold uppercase tracking-wider text-zinc-600">Total</div><div className="text-sm font-semibold text-orange-300">{money(currentSaleTotal)}</div></div>} />
...
function TireSalesReportPage({ showToast, setPage }: { showToast: (m: string) => void; setPage: (p: Page) => void }) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><div className="mb-1 text-[9px] font-black uppercase tracking-[.2em] text-orange-500">Performance ledger</div><h1 className="text-2xl font-black uppercase tracking-[-.045em] text-white sm:text-3xl">Sales Reports</h1><p className="mt-1 text-xs text-zinc-600">Monthly tire sales, services, and payment totals.</p>{lastUpdated && <p className="mt-1 text-[10px] text-zinc-700">Updated {lastUpdated.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}</p>}</div><div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap"><Button className="justify-center" variant="ghost" disabled={refreshing} onClick={() => void load()}><RefreshCw className={`h-3.5 w-3.5 ${refreshing ? "animate-spin" : ""}`} /> Refresh</Button><Button className="justify-center" variant="secondary" onClick={() => setPage("tire-sales")}><ShoppingCart className="h-3.5 w-3.5 text-orange-500" /> Record Work</Button></div></div>

      <section className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-4 sm:p-5">
        <div className="flex items-center justify-between gap-3"><button aria-label="Previous month" onClick={() => setMonthKey((current) => shiftMonthKey(current, -1))} className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-zinc-800 bg-zinc-950 text-zinc-300 transition hover:border-zinc-600 hover:text-white"><ChevronRight className="h-5 w-5 rotate-180" /></button><div className="min-w-0 text-center"><div className="text-[10px] font-semibold uppercase tracking-wider text-orange-400">Viewing month</div><h2 className="mt-1 truncate text-xl font-semibold text-white sm:text-2xl">{monthKeyLabel(monthKey)}</h2></div><button aria-label="Next month" disabled={!canGoForward} onClick={() => setMonthKey((current) => shiftMonthKey(current, 1))} className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-zinc-800 bg-zinc-950 text-zinc-300 transition hover:border-zinc-600 hover:text-white disabled:cursor-not-allowed disabled:opacity-30"><ChevronRight className="h-5 w-5" /></button></div>
      </section>
...
function Toast({ message }: { message: string }) {
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="fixed bottom-24 left-4 right-4 z-50 border-l-2 border-orange-500 bg-[#151615] px-4 py-3 text-center text-xs font-bold text-zinc-200 shadow-2xl sm:left-auto sm:right-5 sm:max-w-sm lg:bottom-5">
      {message}
    </motion.div>
  );
}
...
function Card({ children }: { children: React.ReactNode }) {
  return <div className="rounded-xl border border-white/[.07] bg-[#101110]/85 p-4 shadow-[0_18px_50px_rgba(0,0,0,.18)] sm:p-5">{children}</div>;
}
...
function CardHeader({ title, icon, action }: { title: string; icon: React.ReactNode; action?: React.ReactNode }) {
  return (
    <div className="flex flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-center gap-2">
        {icon}
        <h2 className="text-sm font-black uppercase tracking-[-.01em] text-zinc-100">{title}</h2>
      </div>
      {action}
    </div>
  );
...
function Button({ children, variant = "primary", onClick, disabled, className = "" }: { children: React.ReactNode; variant?: "primary" | "secondary" | "ghost" | "danger"; onClick?: () => void; disabled?: boolean; className?: string }) {
  const styles = {
    primary: "bg-orange-500 font-black text-black hover:bg-orange-400",
...
function Checklist({ label, done }: { label: string; done: boolean }) {
  return (
    <div className="flex items-center gap-2 text-xs text-zinc-300">
      <CheckCircle2 className={`h-3.5 w-3.5 ${done ? "text-orange-400" : "text-zinc-600"}`} />
      <span>{label}</span>
    </div>
  );
}
  ...
  return (
    <footer className="mt-auto hidden border-t border-white/[.06] bg-[#0a0c0b] py-5 text-center text-[10px] text-zinc-700 lg:block">
      <div className="mx-auto flex max-w-[1240px] flex-col items-center justify-between gap-2 px-4 sm:flex-row sm:px-6 lg:px-8">
        <div className="font-black uppercase tracking-[.16em] text-zinc-600">Akron Tire Shop <span className="text-orange-500">/</span> Shop System</div>
        <div className="flex items-center gap-2 font-bold uppercase tracking-[.12em]"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" /> Records save automatically</div>
      </div>
    </footer>
  );
}
