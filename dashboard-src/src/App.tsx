...
function printSaleReceipt(sale: TireSale, onBlocked: () => void) {
  ...
  receiptWindow.document.write(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Akron Tire Shop Receipt ${printEscape(receiptNumber)}</title><style>
    @page{size:80mm auto;margin:5mm}*{box-sizing:border-box}body{margin:0;background:#eef0ef;color:#171917;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif;font-size:12px}.invoice{width:min(420px,100%);margin:22px auto;background:#fff;border:1px solid #d9ddda;border-radius:12px;box-shadow:0 18px 50px rgba(14,20,16,.12);overflow:hidden}.header{padding:27px 26px 23px;border-top:5px solid #1abf7a}.brand{font-size:20px;font-weight:900;letter-spacing:-.035em}.tagline{margin-top:3px;color:#747c77;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.12em}.title-row{display:flex;align-items:flex-end;justify-content:space-between;gap:18px;margin-top:28px}.invoice-title{margin:0;font-size:28px;font-weight:800;letter-spacing:-.045em}.status{padding:5px 9px;border:1px solid #a8dfc5;border-radius:5px;background:#effaf4;color:#11764c;font-size:8px;font-weight:900;letter-spacing:.12em}.content{padding:0 26px 26px}.info-grid{display:grid;grid-template-columns:1fr 1fr;gap:18px;padding:19px 0;border-top:1px solid #e1e4e2;border-bottom:1px solid #e1e4e2}.info-block:last-child{text-align:right}.label{font-size:8px;font-weight:850;text-transform:uppercase;letter-spacing:.13em;color:#8a918d}.value{margin-top:5px;font-size:11px;font-weight:750}.subvalue{margin-top:3px;color:#737b76;font-size:9px}.items{width:100%;margin-top:22px;border-collapse:collapse}.items th{padding:0 0 8px;border-bottom:1px solid #cfd4d1;color:#8a918d;font-size:8px;font-weight:850;text-align:left;text-transform:uppercase;letter-spacing:.11em}.items th:nth-child(2),.items td:nth-child(2){width:42px;text-align:center}.items th:last-child,.items td:last-child{text-align:right}.items td{padding:16px 0;border-bottom:1px solid #e2e5e3;vertical-align:top}.items strong{display:block;font-size:14px}.items span{display:block;margin-top:4px;color:#707873;font-size:9px}.items td:last-child{font-size:14px;font-weight:850}.details{display:grid;gap:9px;margin-top:16px;padding-bottom:16px;border-bottom:1px solid #e2e5e3}.detail{display:flex;justify-content:space-between;gap:18px;color:#747c77;font-size:10px}.detail b{max-width:65%;color:#242824;text-align:right}.total{display:flex;align-items:flex-end;justify-content:space-between;margin-top:18px;padding-top:2px}.total-label{font-size:10px;font-weight:850;text-transform:uppercase;letter-spacing:.1em}.total-value{font-size:29px;font-weight:900;letter-spacing:-.045em}.thank-you{margin-top:28px;padding-top:16px;border-top:1px solid #e1e4e2;text-align:center}.thank-you b{display:block;font-size:11px}.thank-you span{display:block;margin-top:4px;color:#858d88;font-size:9px}.actions{display:flex;gap:9px;width:min(420px,calc(100% - 24px));margin:0 auto 22px}.actions button{flex:1;border:0;border-radius:9px;padding:14px;font:inherit;font-weight:800;cursor:pointer}.print{background:#171917;color:#fff}.close{border:1px solid #cdd2cf!important;background:#fff;color:#343834}.print-note{width:min(420px,calc(100% - 24px));margin:-10px auto 14px;color:#66706a;font-size:10px;text-align:center}@media(max-width:460px){body{background:#fff}.invoice{width:100%;margin:0;border:0;border-radius:0;box-shadow:none}.actions{position:sticky;bottom:0;width:100%;margin:0;padding:12px;background:rgba(255,255,255,.96);backdrop-filter:blur(10px)}.print-note{width:100%;margin:10px 0 0;padding:0 14px}}@media print{body{background:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact}.invoice{width:100%;margin:0;border:0;border-radius:0;box-shadow:none}.header{padding:18px 16px}.content{padding:0 16px}.actions,.print-note{display:none}}
  </style></head><body><main class="invoice"><header class="header"><div class="brand">Akron Tire Shop</div><div class="tagline">Tires &amp; Automotive Service</div><div class="title-row"><h1 class="invoice-title">Receipt</h1><span class="status">PAID</span></div></header><div class="content"><section class="info-grid"><div class="info-block"><div class="label">Billed to</div><div class="value">${printEscape(sale.customer || "Walk-in customer")}</div><div class="subvalue">${printEscape(workName)}</div></div><div class="info-block"><div class="label">Receipt details</div><div class="value">#${printEscape(receiptNumber)}</div><div class="subvalue">${printEscape(soldDate)} · ${printEscape(soldTime)}</div></div></section><table class="items"><thead><tr><th>Description</th><th>Qty</th><th>Amount</th></tr></thead><tbody>${itemDetails}</tbody></table><section class="details"><div class="detail"><span>Payment method</span><b>${printEscape(sale.paymentMethod)}</b></div>${sale.notes ? `<div class="detail"><span>Details</span><b>${printEscape(sale.notes)}</b></div>` : ""}</section><div class="total"><div class="total-label">Total Paid</div><div class="total-value">${printEscape(money(sale.total))}</div></div><div class="thank-you"><b>Thank you for choosing Akron Tire Shop.</b><span>Keep this receipt for your records.</span></div></div></main><p class="print-note">Tap the button below, then choose a printer or Save as PDF.</p><div class="actions"><button class="close" onclick="window.close()">Close</button><button class="print" onclick="window.focus();window.print()">Print / Save PDF</button></div></body></html>`);
