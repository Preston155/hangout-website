    ...
    <div className="min-h-screen bg-[#090b0a] font-sans text-zinc-100 antialiased selection:bg-emerald-500/25 selection:text-white">
      <div className="flex min-h-screen">
        ...
        <main className="flex min-w-0 flex-1 flex-col lg:pl-64">
          <Topbar page={page} />
          <div className="mx-auto w-full max-w-[1280px] flex-1 px-3.5 pb-32 pt-4 sm:px-6 sm:pb-28 sm:pt-7 lg:px-8 lg:pb-10 lg:pt-8">
...
function Topbar({ page }: { page: Page }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 items-center border-b border-white/[.06] bg-[#090b0a]/92 px-3.5 shadow-[0_8px_28px_rgba(0,0,0,.12)] backdrop-blur-xl sm:h-16 sm:px-6 lg:shadow-none lg:px-8">
      <div className="flex items-center gap-2.5 lg:hidden"><div className="grid h-8 w-8 place-items-center rounded-xl border border-emerald-400/15 bg-emerald-400/[.07] text-[10px] font-bold text-emerald-300">AT</div><div><div className="text-xs font-semibold tracking-tight text-white">Akron Tire Shop</div><div className="mt-0.5 text-[9px] text-zinc-600">Shop management</div></div></div>
...
function MobileShopNav({ page, setPage }: { page: Page; setPage: (p: Page) => void }) {
  return (
    <nav className="fixed inset-x-3 bottom-[max(10px,env(safe-area-inset-bottom))] z-40 rounded-2xl border border-white/[.09] bg-[#121513]/94 p-1.5 shadow-[0_18px_55px_rgba(0,0,0,.5)] backdrop-blur-2xl lg:hidden">
      <div className="mx-auto grid max-w-lg grid-cols-5 gap-1">
        {navItems.map((item) => { const Icon = item.icon; const active = item.id === page; return <motion.button whileTap={{ scale: 0.9 }} key={item.id} onClick={() => setPage(item.id)} className={`relative flex min-w-0 flex-col items-center gap-1 overflow-hidden rounded-xl px-1 py-2.5 text-[9px] font-medium ${active ? "text-emerald-200" : "text-zinc-600"}`}>{active && <motion.span layoutId="mobile-shop-active" className="absolute inset-0 rounded-xl border border-emerald-400/15 bg-emerald-400/[.11]" transition={{ type: "spring", stiffness: 420, damping: 34 }} />}<motion.span className="relative" animate={{ y: active ? -1 : 0, scale: active ? 1.1 : 1 }} transition={{ type: "spring", stiffness: 420, damping: 28 }}><Icon className={`h-[19px] w-[19px] ${active ? "text-emerald-300" : "text-zinc-600"}`} /></motion.span><span className="relative truncate">{item.shortLabel}</span></motion.button>; })}
      </div>
    </nav>
  );
}
...
function SettingsPage({ showToast }: { showToast: (m: string) => void }) {
  ...
  return (
    <div className="space-y-6">
      <ShopPageHeader eyebrow="Security" title="Account" description="Manage secure access to Akron Tire Shop." />
...
function ShopPageHeader({ eyebrow, title, description, meta, actions }: { eyebrow: string; title: string; description: string; meta?: React.ReactNode; actions?: React.ReactNode }) {
  return (
    <div className="mobile-surface flex flex-col gap-4 rounded-2xl border border-white/[.065] bg-[#111412] p-4 sm:rounded-none sm:border-x-0 sm:border-t-0 sm:bg-transparent sm:px-0 sm:pb-6 sm:pt-0 sm:shadow-none sm:flex-row sm:items-end sm:justify-between">
...
function TireInventoryPage({ showToast, setPage }: { showToast: (m: string) => void; setPage: (p: Page) => void }) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4 [&>*:last-child]:col-span-2 sm:[&>*:last-child]:col-span-1">
