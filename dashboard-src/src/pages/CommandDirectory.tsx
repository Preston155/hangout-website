...
export function CommandDirectory() {
  ...
  return (
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-[38px] border border-white/[.09] bg-[#090a10]/72 p-5 shadow-[0_50px_140px_-72px_rgba(0,0,0,.95)] backdrop-blur-2xl sm:p-7 lg:p-8" style={{ "--accent": activeBot.accent } as React.CSSProperties}>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.15fr_.85fr] xl:items-stretch">
          <div className="flex flex-col justify-between gap-8">
            <div>
              ...
              <h1 className="mt-5 max-w-4xl text-[clamp(38px,6vw,84px)] font-black leading-[.9] tracking-[-.07em] text-white">
                Control every bot from one command cockpit.
              </h1>
              <p className="mt-5 max-w-2xl text-[15px] leading-7 text-zinc-400">
                A faster, cleaner command directory for IceSway, Veltrix, and ECRP — built around real bot folders, searchable usage, permissions, categories, and live runtime context.
              </p>
            </div>
...
function RuntimePanel({ botName, pm2, prefix, status, online, accent, total }: { botName: string; pm2: string; prefix: string; status: string; online: boolean; accent: string; total: number }) {
  return <div className="relative overflow-hidden rounded-[28px] border border-white/10 bg-white/[.045] p-5"><div className="absolute -right-12 -top-12 h-32 w-32 rounded-full opacity-25 blur-3xl" style={{ background: accent }} /><div className="relative flex items-center justify-between"><div><div className="text-[11px] font-black uppercase tracking-[.18em] text-zinc-500">Active bot</div><div className="mt-1 text-2xl font-black text-white">{botName}</div></div><span className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-[12px] font-black ${online ? "border-emerald-400/20 bg-emerald-400/10 text-emerald-300" : "border-rose-400/20 bg-rose-400/10 text-rose-300"}`}><span className={`h-2 w-2 rounded-full ${online ? "bg-emerald-400" : "bg-rose-400"}`} />{online ? "Online" : status}</span></div><div className="relative mt-5 grid grid-cols-3 gap-2"><BotMicro label="PM2" value={pm2} /><BotMicro label="Prefix" value={prefix} /><BotMicro label="Commands" value={total} /></div></div>;
}
function Spotlight({ command, accent }: { command: BotCommand; accent: string }) { return <div className="relative overflow-hidden rounded-[28px] border border-white/10 bg-black/22 p-5"><div className="absolute -right-10 -top-10 h-28 w-28 rounded-full opacity-25 blur-2xl" style={{ background: accent }} /><div className="relative flex items-center justify-between"><span className="text-[11px] font-black uppercase tracking-[.18em] text-zinc-500">Command spotlight</span><Terminal size={18} style={{ color: accent }} /></div><h3 className="relative mt-3 break-all font-mono text-xl font-black text-white">{command.name}</h3><p className="relative mt-2 line-clamp-2 text-[13px] leading-6 text-zinc-400">{command.description}</p><code className="relative mt-4 block rounded-2xl border border-white/[.07] bg-white/[.035] px-3 py-2 font-mono text-[12px] text-zinc-200">{command.usage}</code></div>; }
