...
function botPowerSummary(botId) {
  const status = state.botPower?.[botId];
  if (!status) return { label: "Status unknown", cls: "unknown", meta: "PM2 status not checked" };
  const online = status.status === "online" || status.online;
  const mem = Number.isFinite(Number(status.memoryMb)) ? ` • ${status.memoryMb} MB` : "";
  return {
    label: online ? "Online" : (status.status === "stopped" ? "Offline" : status.status || "Unknown"),
    cls: online ? "online" : (status.status === "stopped" ? "offline" : "unknown"),
    meta: `${status.pm2Name || botId}${mem}`,
  };
}

function adminApiPassword() {
  return sessionStorage.getItem("vx_admin_pass") || ADMIN_PASS;
}

function syncBotPowerStatuses(statuses = []) {
  for (const status of statuses) state.botPower[status.id] = status;
  for (const status of statuses) {
    const summary = botPowerSummary(status.id);
    document.querySelectorAll(`[data-power-state="${status.id}"]`).forEach((node) => {
      node.className = `admin-power-state admin-power-state--${summary.cls}`;
      node.innerHTML = `<i></i>${esc(summary.label)}<small>${esc(summary.meta)}</small>`;
    });
  }
}

async function setBotPower(botId, action) {
  document.querySelectorAll(`[data-bot-power="${botId}"]`).forEach((btn) => { btn.disabled = true; });
  showToast(`${action === "status" ? "Checking" : action[0].toUpperCase() + action.slice(1)} ${botId}...`);
  try {
    const password = adminApiPassword();
    const res = await fetch(`${API_BASE}/api/admin/bot-power`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-Admin-Pass": password },
      body: JSON.stringify({ password, botId, action }),
    });
    const payload = await res.json().catch(() => ({}));
    if (!res.ok || payload.ok === false) throw new Error(payload.error || `Request failed (${res.status})`);
    syncBotPowerStatuses(payload.data?.statuses || []);
    const target = payload.data?.target?.name || botId;
    showToast(action === "status" ? `${target} status refreshed` : `${target} ${action} sent`);
  } catch (error) {
    showToast(error.message || "Bot power request failed");
  } finally {
    document.querySelectorAll(`[data-bot-power="${botId}"]`).forEach((btn) => { btn.disabled = false; });
  }
}

function refreshAdminBotPower() {
  if (!state.adminAuth) return;
  for (const bot of allBotSummaries()) setBotPower(bot.id, "status");
}
...
function renderAdminBotCards(summaries) {
  return summaries.map((bot) => `<article class="admin-bot-card${bot.id === state.activeBot ? ' is-active' : ''}">
    ...
    ${(() => { const power = botPowerSummary(bot.id); return `<div class="admin-power-box">
      <div class="admin-power-state admin-power-state--${power.cls}" data-power-state="${esc(bot.id)}"><i></i>${esc(power.label)}<small>${esc(power.meta)}</small></div>
      <div class="admin-power-actions">
        <button class="admin-power-btn admin-power-btn--start" data-bot-power="${esc(bot.id)}" data-bot-action="start" type="button">Start</button>
        <button class="admin-power-btn admin-power-btn--stop" data-bot-power="${esc(bot.id)}" data-bot-action="stop" type="button">Stop</button>
        <button class="admin-power-btn" data-bot-power="${esc(bot.id)}" data-bot-action="restart" type="button">Restart</button>
      </div>
    </div>`; })()}
...
function wireShellEvents() {
  ...
  app.addEventListener("click", (e) => {
    ...
    const powerBtn = e.target.closest('[data-bot-power][data-bot-action]');
    if (powerBtn) {
      e.preventDefault();
      e.stopPropagation();
      setBotPower(powerBtn.dataset.botPower, powerBtn.dataset.botAction);
      return;
    }
